...
class Mtaa_Portfolio {
    public function __construct() {
        ...
        add_filter( 'mtaa_option_files', array ( $this, 'register_options' ) );
        add_filter( 'mtaa_field_save_portfolio_root', array ( $this, 'save_portfolio_root' ) );
    ...
    public function register_post_types() {
        $args = array(
            ...
            'labels' => array(
                'name'               => __( 'Portfolio Items',                   'mtaa' ),
                'singular_name'      => __( 'Portfolio Item',                    'mtaa' ),
                'menu_name'          => __( 'Portfolio',                         'mtaa' ),
                'name_admin_bar'     => __( 'Portfolio Item',                    'mtaa' ),
                'add_new'            => __( 'Add New',                           'mtaa' ),
                'add_new_item'       => __( 'Add New Portfolio Item',            'mtaa' ),
                'edit_item'          => __( 'Edit Portfolio Item',               'mtaa' ),
                'new_item'           => __( 'New Portfolio Item',                'mtaa' ),
                'view_item'          => __( 'View Portfolio Item',               'mtaa' ),
                'search_items'       => __( 'Search Portfolio',                  'mtaa' ),
                'not_found'          => __( 'No portfolio items found',          'mtaa' ),
                'not_found_in_trash' => __( 'No portfolio items found in trash', 'mtaa' ),
                'all_items'          => __( 'Portfolio Items',                   'mtaa' ),

                // Custom labels b/c Mtaandao doesn't have anything to handle this.
                'archive_title'      => __( 'Portfolio',                         'mtaa' ),
            )
        );
    ...
    public function register_taxonomies() {
        ...
        $args = array(
            ...
            'labels' => array(
                'name'                       => __( 'Portfolios',                           'mtaa' ),
                'singular_name'              => __( 'Portfolio',                            'mtaa' ),
                'menu_name'                  => __( 'Portfolios',                           'mtaa' ),
                'name_admin_bar'             => __( 'Portfolio',                            'mtaa' ),
                'search_items'               => __( 'Search Portfolios',                    'mtaa' ),
                'popular_items'              => __( 'Popular Portfolios',                   'mtaa' ),
                'all_items'                  => __( 'All Portfolios',                       'mtaa' ),
                'edit_item'                  => __( 'Edit Portfolio',                       'mtaa' ),
                'view_item'                  => __( 'View Portfolio',                       'mtaa' ),
                'update_item'                => __( 'Update Portfolio',                     'mtaa' ),
                'add_new_item'               => __( 'Add New Portfolio',                    'mtaa' ),
                'new_item_name'              => __( 'New Portfolio Name',                   'mtaa' ),
                'separate_items_with_commas' => __( 'Separate portfolios with commas',      'mtaa' ),
                'add_or_remove_items'        => __( 'Add or remove portfolios',             'mtaa' ),
                'choose_from_most_used'      => __( 'Choose from the most used portfolios', 'mtaa' ),
            )
        );
    ...
    public function save_post( $post_id ) {
        ...
        if ( $parent_id = mn_is_post_revision( $post_id ) ) {
            $post_id = $parent_id;
        }
    ...
    public function locate_template( $type, $selected = false ) {
        ...
        if ( $selected ) {
            ...
            foreach ( $paths as $path ) {
                $template_info = Mtaa::get_file_data( $path, array( 'Portfolio Style' ) );
    ...
    public static function get_available_templates( $type ) {
        ...
        foreach ( $paths as $path ) {
            $file_data = Mtaa::get_file_data( $path, array( 'Portfolio Style' ) );
            $names[] = $file_data[0];
        }
        ...
        if ( $post_type === 'portfolio_item' ) { ?>
            <style type="text/css">
                #icon-edit.icon32-posts-portfolio_item {
                    background: transparent url( '<?php echo Mtaa::$assetsPath . '/images/components/portfolio/portfolio-32.png'; ?>' ) no-repeat;
                }
            </style>
        <?php }
    }
    ...
    /**
     * Register component options for mtaa framework.
     *
     * @param  array $mtaa_options
     * @return array
     */
    public function register_options( $mtaa_options ) {
        $mtaa_options[] = sprintf( '%s/options.php', dirname( __FILE__ ) );
