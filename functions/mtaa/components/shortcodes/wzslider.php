...
class wzslider {
    ...
    static public function init($atts, $content = null, $code = '') {
        ...
        if ($attachments) {
            ...
            foreach ($attachments as $attachment) {
                ...
                $url = mn_get_attachment_image_src($attachment->ID, 'large');
                $url = $url[0];

                $thumb = mn_get_attachment_image_src($attachment->ID, 'wzslider-thumbnail');
                $thumb = $thumb[0];
    ...
    static public function loadStatic() {
        mn_enqueue_script('galleria', Mtaa::$assetsPath . '/js/galleria.js', array('jquery'), null, true);
        mn_enqueue_script('wzslider', Mtaa::$assetsPath . '/js/wzslider.js', array('jquery'), null, true);
     }

    static public function loadStyles() {
        mn_register_style('wzslider', Mtaa::$assetsPath . '/css/wzslider.css');
        mn_enqueue_style('wzslider');
    }
    ...
    static public function check($posts) {
        ...
        if ($found) {
            add_action('mn_footer', 'wzslider::galleriaScript');

            add_action('mn_enqueue_scripts', 'wzslider::loadStatic');
            add_action('mn_enqueue_scripts', 'wzslider::loadStyles');
        }
...
function add_slider_tinymce_plugin($plugin_array) {
   $plugin_array['wzslider'] = Mtaa::$assetsPath . '/js/wzslider_button.js';
