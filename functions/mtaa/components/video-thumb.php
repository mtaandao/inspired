...
class Mtaa_Video_Thumb extends Mtaa_Video_API {
	public static function init() {
		if(current_user_can('edit_posts')) {
			add_action('mn_ajax_zoom_autothumb_get', array(__CLASS__, 'admin_ajax_thumb_get'));
			if(current_user_can('upload_files')) add_action('mn_ajax_zoom_autothumb_attach', array(__CLASS__, 'admin_ajax_thumb_attach'));
			add_action('admin_head-post-new.php', array(__CLASS__, 'admin_nemnost_head'), 100);
			add_action('admin_head-post.php', array(__CLASS__, 'admin_nemnost_head'), 100);
			add_action('admin_footer-post-new.php', array(__CLASS__, 'admin_nemnost_foot'), 100);
			add_action('admin_footer-post.php', array(__CLASS__, 'admin_nemnost_foot'), 100);
		}
	}
	...
	public static function admin_ajax_thumb_get() {
		if (isset($_POST['mtaa_autothumb_embedcode']) && isset($_POST['mtaa_autothumb_postid'])) {
			$url = parent::extract_url_from_embed(trim(stripslashes($_POST['mtaa_autothumb_embedcode'])));
			$postid = intval($_POST['mtaa_autothumb_postid']);
	...
	public static function admin_ajax_thumb_attach() {
		if(isset($_POST['mtaa_autothumb_embedcode']) && isset($_POST['mtaa_autothumb_postid']) && isset($_POST['mtaa_autothumb_nonce'])) {
			$url = parent::extract_url_from_embed(trim(stripslashes($_POST['mtaa_autothumb_embedcode'])));
			$postid = intval($_POST['mtaa_autothumb_postid']);
			$nonce = trim($_POST['mtaa_autothumb_nonce']);

			if(empty($url) || filter_var($url, FILTER_VALIDATE_URL) === false || $postid < 1 || !mn_verify_nonce($nonce, 'mtaa_attach_thumbnail-' . $postid)) return false;
	 ...
	 */
	public static function admin_nemnost_head() {
		?><style type="text/css">
			#layout_select1 { position: relative; }
			#mnz_autothumb { position: relative; }
			#mtaa_autothumb_preview { display: none; text-align: center; margin: 0; }
			#normal-sortables #mtaa_autothumb_preview { display: none; text-align: left; margin: 0; }
			#mtaa_autothumb_preview img { display: block; width: 100%; margin-bottom: 5px; }
			#normal-sortables #mtaa_autothumb_preview img { display: block; width: 300px; margin-bottom: 5px; }
			#mtaa_autothumb_preview #mnz_usethis { display: inline-block; text-align: center; line-height: 15px; padding: 3px 10px; cursor: pointer; }
			#normal-sortables #mtaa_autothumb_preview #mnz_usethis { display: inline-block; text-align: left; line-height: 15px; padding: 3px 10px; cursor: pointer; }
			#mtaa_autothumb_preview #mnz_usethis.button-disabled { cursor: default; }
			#mnz_autothumb_error { background-color: #ffffe0; padding: 5px 8px; border: 1px solid #e6db55; -webkit-border-radius: 3px; -moz-border-radius: 3px; border-radius: 3px; margin: 0; }
			#mnz_autothumb_error strong { display: inline-block; margin-bottom: 3px; }
			#mnz_autothumb_error small#mnz_autothumb_remind { display: block; line-height: 1.5; color: #a19a3c; margin-top: 8px; }
			#mnz_ajax_loading { display: block; position: absolute; left: 0; right: 0; z-index: 100; text-align: center; text-shadow: 0 0 5px #fff, 0 0 5px #fff, 0 0 5px #fff, 0 0 5px #fff, 0 0 5px #fff; color: #000; }
		</style><?php
	}
	 ...
	 */
	public static function admin_nemnost_foot() {
		?><script type="text/javascript">
			var mnzVideoUrlInputTimeout,
			...
			jQuery(function($){
				$('<div id="mnz_autothumb"><small id="mnz_ajax_loading" style="display:none">Loading...</small><p id="mnz_autothumb_error" class="updated" style="display:none"><?php _e('<strong>NOTICE!&nbsp;&nbsp;Unable to fetch video thumbnail</strong><br/>Either an invalid embed code was provided, or there is no thumbnail available for the specified video&hellip;<br/><small id="mnz_autothumb_remind"><strong>REMINDER:</strong> You can always manually upload a featured image via the Mtaandao Media Uploader.</small>', 'mtaa'); ?></p><p id="mtaa_autothumb_preview"><img src=""/><small id="mnz_usethis" class="button">Use This as the Featured Image</small></p></div>')
					.insertAfter('#mtaa_post_embed_code');

				$('#mtaa_autothumb_preview img').on('load', function(){
					$('#mnz_ajax_loading, #mnz_autothumb_error').hide();
					$('#mtaa_autothumb_preview').animate({height:'show',opacity:'show'});
				});

				$('#mtaa_post_embed_code').on('input', function(){
					clearTimeout(mnzVideoUrlInputTimeout);

					if('' != (val = $.trim($('#mtaa_post_embed_code').val())) && mnzValidIframeRegex.test(val)) {
						$('#mnz_ajax_loading').fadeIn();

						mnzVideoUrlInputTimeout = setTimeout(MNZVideoUrlOnInput, 1000);
					} else {
						$('#mnz_ajax_loading, #mnz_autothumb_error').hide();

						MNZRemovePreviewImage();
					}
				}).triggerHandler('input');

				$('#mnz_usethis').on('click', MNZUseThisOnClick);

				$('#postimagediv').on('click', '#remove-post-thumbnail', MNZEnableSetFeaturedButton);
			});

			// This is called onInput if an actual embed code is provided
			function MNZVideoUrlOnInput() {
				var embed = jQuery.trim(jQuery('#mtaa_post_embed_code').val());
				
				jQuery.ajax({
					url: ajaxurl,
					type: 'post',
					data: { action: 'mtaa_autothumb_get', mtaa_autothumb_embedcode: embed, mtaa_autothumb_postid: jQuery('#post_ID').val() },
					dataType: 'json',
					complete: function(xhr,status){
						...
						if(response == 'ERROR') {
							MNZRemovePreviewImage();
							jQuery('#mnz_ajax_loading').hide();
							jQuery('#mnz_autothumb_error').show();
							return;
						}
						...
						if(thumb_url != '') {
							jQuery('#mtaa_autothumb_preview img').attr('src', '' + thumb_url);

							if(is_featured == true) {
								MNZDisableSetFeaturedButton();
							} else {
								MNZEnableSetFeaturedButton();
							}
						} else {
							MNZRemovePreviewImage();
						}
			...
			function MNZUseThisOnClick() {
				if('' != (embed = jQuery.trim(jQuery('#mtaa_post_embed_code').val()))) {
					jQuery.ajax({
						url: ajaxurl,
						type: 'post',
						data: {
						        action: 'mtaa_autothumb_attach',
						        mtaa_autothumb_embedcode: embed,
						        mtaa_autothumb_postid: jQuery('#post_ID').val(),
						        mtaa_autothumb_nonce: '<?php global $post; echo mn_create_nonce('mtaa_attach_thumbnail-' . $post->ID); ?>'
									},
						dataType: 'text',
						complete: function(xhr,status){
							...
							if(response != 'false' && response > 0) {
								MNZSetAsThumbnail(response, '<?php echo mn_create_nonce('set_post_thumbnail-' . $post->ID); ?>');
							}
			...
			function MNZEnableSetFeaturedButton() {
				jQuery('#mnz_usethis')
					.html('Use This as the Featured Image')
					.off('click', MNZUseThisOnClick)
					.on('click', MNZUseThisOnClick)
					.removeClass('button-disabled');
			}

			// Disable the "Use This as the Featured Image" button
			function MNZDisableSetFeaturedButton() {
				jQuery('#mnz_usethis')
			...
			function MNZRemovePreviewImage() {
				jQuery('#mtaa_autothumb_preview').hide().removeAttr('src');
			}

			// Almost the same as the core MNSetAsThumbnail function
			function MNZSetAsThumbnail(id, nonce) {
				jQuery.post(ajaxurl, {
	...
	public static function attach_remote_video_thumb($thumb_url, $post_id, $size = null) {
		...
		if(false === ($id=self::thumb_attachment_exists($url, $post_id)) || $id < 1) {
			...
			if(!is_mn_error($id) && $id !== false && $id > 0) {
				add_post_meta($id, '_mnz_original_thumb_url', $url);
				return $id;
			} else {
				return false;
			}
	...
	public static function thumb_attachment_exists($thumb_url, $post_id) {
		...
		$db = get_children(array(
			...
			'meta_key' => '_mnz_original_thumb_url',
	...
	public static function gettheimage_url($post_id) {
		...
		$embed_code = get_post_meta($post_id, 'mtaa_post_embed_code', true);
		...
		$attachment_image_src = mn_get_attachment_image_src($attachment_id);
