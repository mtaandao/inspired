...
class Mtaa_Video_API {
	...
	public static function fetch_extended_video_data($post = null) {
		...
		$embed = trim(get_post_meta($post->ID, 'mtaa_post_embed_code', true));
		...
		$response = mn_remote_get($datauri, array('sslverify' => false));
		if(is_mn_error($response)) return false;

		$body = mn_remote_retrieve_body($response);
