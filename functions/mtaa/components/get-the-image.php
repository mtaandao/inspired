 ...
 *
 * Get the Image - An advanced post image script for Mtaandao.
 *
 ...
 * backwards compatibility.  It also supports Mtaandao' built-in featured image functionality.  On top of
 ...
 * @link http://justintadlock.com/archives/2008/05/27/get-the-image-mtaandao-plugin
 * @license http://www.gnu.org/licenses/old-licenses/gpl-2.0.html
 */

/* Adds theme support for Mtaandao 'featured images'. */
...
function get_the_image( $args = array() ) {
    ...
    $defaults = array(
        ...
        'post_id' =>            get_the_ID(),
        'attachment' =>         false,
        'the_post_thumbnail' => true, // MN 2.9+ image function
        'size' =>               'large',
        'default_image' =>      false,
        'order_of_image' =>     1,
        'link_to_post' =>       true,
    ...
    $image_cache = mn_cache_get( $post_id, 'get_the_image' );
    ...
    if ( !isset( $image_cache[$key] ) || empty( $cache ) ) {
        ...
        /* If no image found and $the_post_thumbnail is set to true, check for a post image (MN feature). */
        if ( empty( $image ) && !empty( $the_post_thumbnail ) ) {
            $image = get_the_image_by_post_thumbnail( $args );

             if (!isset($image['src'])) unset($image);
        ...
        /* If no image found, try using the video thumbnail if the post contains a video in the embed field. (Mtaa customization) */
        if ( empty( $image ) ) {
            if ( !class_exists( 'Mtaa_Video_Thumb' ) ) require_once Mtaa_INC . '/components/video-thumb.php';
            $mnzthumburl = Mtaa_Video_Thumb::gettheimage_url( $post_id );
            if ( $mnzthumburl !== false ) $image = $mnzthumburl;
        }
        ...
        if ( !empty( $image ) ) {
            ...
            mn_cache_set( $post_id, $image_cache, 'get_the_image' );
        }
    }
    ...
    if ( 'array' == $format ) {
        ...
        $atts = mn_kses_hair( $image_html, array( 'http' ) );
...
/**
 * Checks for images using a custom version of the Mtaandao 2.9+ get_the_post_thumbnail() function.
 * If an image is found, return it and the $post_thumbnail_id.  The Mtaandao function's other filters are
...
function get_the_image_by_post_thumbnail( $args = array() ) {
    ...
    /* Check for a post image ID (set by MN as a custom field). */
    ...
    /* Apply filters on post_thumbnail_size because this is a default MN filter used with its image feature. */
    $size = apply_filters( 'post_thumbnail_size', $args['size'] );

    /* Get the attachment image source.  This should return an array. */
    $image = mn_get_attachment_image_src( $post_thumbnail_id, $size );
...
function get_the_image_by_attachment( $args = array() ) {
    ...
    if ( 'attachment' == $post_type && mn_attachment_is_image( $args['post_id'] ) ) {
    ...
    if ( !empty( $attachment_id ) ) {
        ...
        $image = mn_get_attachment_image_src( $id, $args['size'] );
...
function get_the_image_by_scan( $args = array() ) {
    ...
    $videocode = get_post_meta($args['post_id'], 'mtaa_post_embed_code', true);

    if (function_exists('fetch_video_thumbnail_url')) {
    ...
    if ( isset( $matches ) && isset($matches[1][0]) && $matches[1][0] )
...
function get_the_image_format( $args = array(), $image = false ) {
    ...
    /* If there is a $post_thumbnail_id, apply the MN filters normally associated with get_the_post_thumbnail(). */
...
function get_the_image_delete_cache_by_post( $post_id ) {
    mn_cache_delete( $post_id, 'get_the_image' );
}
