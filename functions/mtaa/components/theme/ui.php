...
class ui {
    ...
    public static function title() {
        ...
        if ( is_front_page() && is_home() ) {
            ...
            if (option::get('seo_posts_title') == 'Site Title - Page Title') echo get_bloginfo('name').option::get('title_separator').mn_title('',false,'');
            if (option::get('seo_posts_title') == 'Page Title - Site Title') echo mn_title('',false,'').option::get('title_separator').get_bloginfo('name');
            if (option::get('seo_posts_title') == 'Page Title') echo mn_title('',false,'');
        }
        ...
        if (is_category() || is_archive() || is_search()) {
            if (option::get('seo_pages_title') == 'Site Title - Page Title') echo get_bloginfo('name').option::get('title_separator').mn_title('',false,'');
            if (option::get('seo_pages_title') == 'Page Title - Site Title') echo mn_title('',false,'').option::get('title_separator').get_bloginfo('name');
            if (option::get('seo_pages_title') == 'Page Title') echo mn_title('',false,'');
        }
    }
    ...
    public static function thumbIt( $image, $width, $height, $crop = true ) {
        ...
        if ( is_int( $image ) ) {
            ...
            $result = $mndb->get_var( $mndb->prepare( "SELECT post_id FROM $mndb->postmeta WHERE meta_key = '_mn_attachment_metadata' AND meta_value LIKE %s LIMIT 1;", '%' . like_escape( str_replace( trailingslashit( $base_url ), '', $img_url ) ) . '%' ) );
            $attachment_id = absint( $result ) > 0 ? absint( $result ) : false;
        }

        if ( $attachment_id === false ) return $image;

        $image = mn_get_attachment_url( $attachment_id );

        $attachment_meta = mn_get_attachment_metadata( $attachment_id );
        ...
        if ( $needs_resize ) {
            ...
            if ( !is_mn_error( $resized ) && $resized !== false ) {
                $key = sprintf( 'resized-%dx%d', $width, $height );
                $attachment_meta['sizes'][ $key ] = $resized;
                $image = str_replace( basename( $image ), $resized['file'], $image );
                mn_update_attachment_metadata( $attachment_id, $attachment_meta );

                $backup_sizes = get_post_meta( $attachment_id, '_mn_attachment_backup_sizes', true );
                if ( !is_array( $backup_sizes ) ) $backup_sizes = array();
                $backup_sizes[ $key ] = $resized;
                update_post_meta( $attachment_id, '_mn_attachment_backup_sizes', $backup_sizes );
            }
        }
    ...
    /**
     * Checks if Mtaandao version is greater or equal to $is_ver
     *
     * @param  string  $is_ver version to be checked
     * @return boolean
     */
    public static function is_mn_version($is_ver) {
        include( ABSPATH . RES . '/version.php' ); // $mn_version; // x.y.z

        return version_compare($mn_version, $is_ver, '>=');
    }
    ...
    public static function recognized_font_families($id = 'global') {
        return apply_filters('mtaa_recognized_font_families', array(
            'arial'     => 'Arial',
            'georgia'   => 'Georgia',
            'helvetica' => 'Helvetica',
            'palatino'  => 'Palatino',
    ...
    public static function recognized_google_webfonts_families($id = 'global') {
        return apply_filters('mtaa_recognized_google_webfonts_families', array(
    ...
    public static function recognized_font_styles($id = 'global') {
        return apply_filters('mtaa_recognized_font_styles', array(
            'normal'      => 'Normal',
            'italic'      => 'Italic',
            'bold'        => 'Bold',
