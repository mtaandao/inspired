...
/**
 * Mtaa Framework Options Manager
 *
 * @category Mtaa
 */
class option {
    ...
    public static function getOptionsArray() {
        $option_files = apply_filters( 'mtaa_option_files', array(
            sprintf( '%s/theme/options.php', FUNC_INC),
            sprintf( '%s/options.php',       Mtaa_INC)
        ) );
    ...
    public static function reset() {
        ...
        $query = "DELETE FROM $mndb->options WHERE option_name LIKE '" . self::$prefix . "%'";
        $mndb->query($query);
    ...
    public static function getWidgetOptions() {
            ...
            $q = "SELECT * FROM $mndb->options WHERE option_name LIKE 'widget_%'";
    ...
    public static function setupWidgetOptions($options, $decode = false) {
        ...
        if ( is_array( $sidebars_widgets ) && !empty( $sidebars_widgets ) ) {
            foreach ( $sidebars_widgets as $sidebar_id => $widgets ) {
                    if ( preg_match( '/^_mnzlb-page-[0-9]+-widgets$/i', $sidebar_id ) ) {
        ...
        foreach ( $options as $id => $option ) {
            ...
            if ( $id == 'sidebars_widgets' ) {
                foreach ( $current_value as $sidebar_id => $widgets ) {
                    if ( preg_match( '/^_mnzlb-page-[0-9]+-widgets$/i', $sidebar_id ) ) $keep[ $sidebar_id ] = $widgets;
                }
            } else {
