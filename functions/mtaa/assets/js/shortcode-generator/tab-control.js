function mnzTabMaker(h, i, f) {
    ...
    this.buildSelectControl = function () {
        ...
        this.selectControl = jQuery( "<select></select>").attr( "id", "mnz-tab-select").addClass(f ? f : "" );
    ...
    this.buildTextInputControl = function ( id ) {
        ...
        var labelElement = '<label for="mnz_tab_title">Tab ' + id + ' Title</label>';
        var inputElement = '<input type="text" id="mnz_tab_title_' + id.toString() + '" class="txt input-text" name="mnz_tab_title" />';
    ...
    this.buildTabButtons = function (a) {
        if (this.buttonsControl) {
            ...
            jQuery( 'label[for="mnz_tab_title"]').parents( 'tr').remove();
