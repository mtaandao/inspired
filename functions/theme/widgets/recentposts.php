...
class Wpmtaa_Feature_Posts extends MN_Widget {

	function Wpmtaa_Feature_Posts() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'feature-posts', 'description' => 'A list of posts, optionally filter by category.' );

		/* Widget control settings. */
		$control_ops = array( 'id_base' => 'mtaa-feature-posts' );

		/* Create the widget. */
		$this->MN_Widget( 'mtaa-feature-posts', 'Mtaa: Recent Posts', $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$query_opts = apply_filters('mtaa_query', array(
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => 'Recent Posts', 'category' => 0, 'show_count' => 3, 'show_date' => false, 'show_thumb' => false, 'show_excerpt' => false, 'hide_title' => false, 'read_more' => true, 'excerpt_length' => 150 );
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'read_more' ); ?>">Display read more button</label>
		</p>
...
function mtaa_register_fp_widget() {
	register_widget('Wpmtaa_Feature_Posts');
}
add_action('widgets_init', 'mtaa_register_fp_widget');